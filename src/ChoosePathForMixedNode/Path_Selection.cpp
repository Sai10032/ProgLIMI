...
bool Is_SDNnodes_In_Path(list<Vertex_d>& SDNnodes, PathType path) {
    PathType_iter pb;
    PathType_iter pe;
    list<Vertex_d>::iterator ib;
    list<Vertex_d>::iterator ie;
    path.pop_back();
    if (path.empty()) {
        return false;
    }
...
void Compute_Shortest_Path_From_OneNode(BGraph& bgraph, Vertex_d& s, Vertex_d& d, PathType& path) {
    ...
    Vertex_d next_parent;
    Vertex_d last_parent;

    boost::dijkstra_shortest_paths(
        bgraph, s,
        predecessor_map(boost::make_iterator_property_map(parent.begin(), get(boost::vertex_index, bgraph)))
...
void Change_Path_to_Matrix(BGraph& bgraph, PathType& path, boost::numeric::ublas::matrix<long double>& matrix1) {
    ...
    PathType_iter p_iter1;
    PathType_iter p_iter2;
    Edge_d ed;
    bool Is_Suc = false;
    ...
    for (p_iter1 = path.begin(); p_iter1 != path.end(); p_iter1++) {
        ...
        if (Is_Suc) {
            index = linkindex[ed];
            //			matrix1(0, index) = matrix1(0,index) + 1; //
            matrix1(0, index) = 1;
        } else {
            matrix1(0, index) = 0;
        }
...
int Find_Linear_Independent_Paths_For_One_Node(BGraph& bgraph, std::queue<PathType>& paths,
    ...
    while (!paths.empty()) {
        ...
        // boost::numeric::ublas::matrix<long double> m_temp =
        // boost::numeric::ublas::prod(boost::numeric::ublas::trans(RR),G); R12 = boost::numeric::ublas::prod(m_temp,
        // boost::numeric::ublas::trans(v));
        ...
        if (R22 > 0.000000001) {
            ...
            if (Output_Paths.size() >= num_edges(bgraph)) return -1;

            G.resize(k, v.size2());
            for (unsigned colum = 0; colum < v.size2(); colum++) {
                G(k - 1, colum) = v(0, colum);
                if (v(0, colum) > 0) {
                    G1[colum].push_back(k - 1);
                }
            }
...
void Choose_Shortest_Independent_paths(BGraph& bgraph, multimap<double, PathType>& Indepedent_Paths,
                                       multimap<double, PathType>& Output_Paths) {
    boost::numeric::ublas::matrix<long double> v(1, num_edges(bgraph));
    ...
    for (path_iter = Indepedent_Paths.begin(); path_iter != Indepedent_Paths.end(); path_iter++) {
        ...
        if (path_iter == Indepedent_Paths.begin()) {
            ...
            // boost::numeric::ublas::matrix<long double> m_temp =
            // boost::numeric::ublas::prod(boost::numeric::ublas::trans(RR),G);
            Compute_The_QMatrix();
            ...
            if (R22 > 0.000000001) {
                ...
                if (path_iter->first >= 3) {
                    ...
                    std::queue<PathType> paths1;
                    std::queue<PathType> paths2;
...
void Output_Paths_In_File(BGraph& bgraph, multimap<double, PathType>& Output_Paths, const string& fileresultname) {
    list<Vertex_d> monitors;
    list<Vertex_d> SDNnodes;
    list<Vertex_d>::iterator vb;
    list<Vertex_d>::iterator ve;
    multimap<double, PathType>::iterator path_iter;
    ofstream fout;
    PathType pathtmp;
    PathType_iter pb;
...
void Check_Linear_Independent(BGraph& bgraph, multimap<double, PathType>& Output_Paths, const string& matrixname) {
    // construct edgeID
    ...
    for (path_iter = Output_Paths.begin(); path_iter != Output_Paths.end(); path_iter++) {
        ...
        list<int>::iterator vb;
        list<int>::iterator ve;
...
void Main_Process_For_Path_Selection(const string& filename, const string& fileresultname) {
    BGraph bgraph;
